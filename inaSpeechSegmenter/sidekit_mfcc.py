...
def trfbank(fs, nfft, lowfreq, maxfreq, nlinfilt, nlogfilt, midfreq=1000):
    ...
    # FFT bins (in Hz)
    n_frequences = np.arange(nfft) / (1.0 * nfft) * fs
    ...
    for i in range(nfilt):
        ...
        lid = np.arange(
            np.floor(low * nfft / fs) + 1, np.floor(cen * nfft / fs) + 1, dtype=np.int32
        )
        left_slope = heights[i] / (cen - low)
        rid = np.arange(
            np.floor(cen * nfft / fs) + 1,
            min(np.floor(hi * nfft / fs) + 1, nfft),
            dtype=np.int32,
        )
...
def framing(sig, win_size, win_shift=1, context=(0, 0), pad="zeros"):
    ...
    if pad == "zeros":
        return np.lib.stride_tricks.as_strided(
            np.pad(sig, c, "constant", constant_values=(0,)), shape=shape, strides=strides
        ).squeeze()
    elif pad == "edge":
        return np.lib.stride_tricks.as_strided(
            np.pad(sig, c, "edge"), shape=shape, strides=strides
        ).squeeze()
...
def pre_emphasis(input_sig, pre):
    ...
    if input_sig.ndim == 1:
        return (
            input_sig
            - np.c_[input_sig[np.newaxis, :][..., :1], input_sig[np.newaxis, :][..., :-1]].squeeze()
            * pre
        )
    else:
...
def mfcc(
    ...
    mspec: np.ndarray[tuple[int, int], np.dtype[PARAM_TYPE]] = np.log(
        np.dot(spec, fbank.T)
